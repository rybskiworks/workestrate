---
name: workflow-gleam-debugging-00-orchestration
description: |
  Use only to orchestrate the Gleam debugging workflow. Use when fixing a failing
  build, a runtime crash, a type-checker rejection, or an incorrect runtime
  result. Do not use for implementing new features (use
  workflow-gleam-implementation), reviewing a diff (use workflow-gleam-code-review),
  or restructuring without behavior change. If the fix requires new public API,
  follow this workflow for the fix then workflow-gleam-implementation for the API
  design steps.
...
# Workflow: Gleam Debugging (Orchestration)

This skill orchestrates the Gleam debugging workflow. It is the entry point only.
...
Source workflow doc: `docs/gleam/workflows/debugging.md`.
...
|-------|------------------|---------|
| 01 | `workflow-gleam-debugging-01-reproduce` | Reproduce the issue reliably; capture the exact command, input, environment, target, and full error or crash output. |
| 02 | `workflow-gleam-debugging-02-diagnose` | Categorize the defect (type-check error / Result-Option error model / OTP-actor-or-supervision / FFI-or-interop / target-specific-semantic / logic) and identify the root cause. |
| 03 | `workflow-gleam-debugging-03-fix` | Implement the minimal root-cause fix; do not suppress symptoms. |
| 04 | `workflow-gleam-debugging-04-regression` | Add a regression test that fails before the fix and passes after. |
| 05 | `workflow-gleam-debugging-05-verify` | Run the full check suite (`gleam check`, `gleam test`, `gleam format --check`) on the relevant target(s); report. |
...
- **`workflow-gleam-debugging-01-reproduce` CANNOT be skipped.** Do not proceed
...
- **`workflow-gleam-debugging-02-diagnose` CANNOT be skipped.** A fix without an
  identified category and root cause is symptom suppression. If the category
  cannot be determined, set `outcome: fail` and stop.
- **`workflow-gleam-debugging-03-fix` CANNOT be skipped.** The workflow exists to
  apply a fix.
- **`workflow-gleam-debugging-04-regression` CANNOT be skipped.** Every fix must
  encode its reproduction as a regression test so the defect cannot silently
  return.
- **`workflow-gleam-debugging-05-verify` CANNOT be skipped.** All gates must pass
...
| --- | --- |
| `workflow-gleam-debugging-05-verify` returns `outcome: pass` | `workflow-gleam-validation-00-orchestration` |
...
tests_needed:
  - ...
next_phase: workflow-gleam-debugging-02-diagnose
next_workflow: null
...
Phase `workflow-gleam-debugging-05-verify` additionally returns these
...
proceeding to the fix. After `workflow-gleam-debugging-02-diagnose` returns its
handoff, surface the category and root cause for confirmation before loading
`workflow-gleam-debugging-03-fix`.

```yaml
continuation: suggest-next
after_phase: workflow-gleam-debugging-02-diagnose
before_phase: workflow-gleam-debugging-03-fix
