---
name: workflow-gleam-code-review-01-scope
description: |
  Use only for the scope phase of the Gleam code-review workflow. Understand the
...
# Phase 01: scope (Gleam code review)
...
2. Record: which files changed, what the stated intent is, and whether the
   change touches public API, externals/FFI, OTP/actors/supervision, or
   dependencies.
3. Identify risk areas (e.g. `@external` declarations, public API surface,
   dependency additions, OTP boundaries) that will drive which docs and skills
   to consult in phases 02-analyze and 04-review.
4. Apply scope discipline: review only the diff; do not request changes to
   untouched code. Pre-existing issues in untouched code are recorded as
   separate follow-ups, not as review findings.

## Docs to consult

None mandatory in this phase (docs load in 02-analyze and 04-review).
Optionally consult `docs/gleam/conventions-patterns-antipatterns.md` if the diff
touches the public API, to prime the risk assessment.
...
- Scope discipline — review only the diff; do not request changes to untouched
  code. Pre-existing issues in untouched code are recorded as separate
