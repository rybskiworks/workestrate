---
name: workflow-gleam-implementation-00-orchestration
description: |
  Use only to orchestrate the Gleam implementation workflow. Use when adding new
  Gleam code — a new feature, module, package, public function/type, or first
  ...
  diff (use workflow-gleam-code-review), refactoring without behavior change, or
  fixing a defect (use workflow-gleam-debugging).
...
# Workflow: Gleam Implementation (Orchestration)

This skill orchestrates the Gleam implementation workflow. It is the entry point
for adding new Gleam code that requires scoping, design, implementation, testing,
...
| --- | --- | --- |
| 01 | `workflow-gleam-implementation-01-scope` | Understand requirements, read topic docs, record intended behavior and public surface. |
| 02 | `workflow-gleam-implementation-02-design` | API design, error type design, module structure decisions. |
| 03 | `workflow-gleam-implementation-03-implement` | Write code following Gleam type and Result rules. |
| 04 | `workflow-gleam-implementation-04-test` | Write tests alongside code, run `gleam test`. |
| 05 | `workflow-gleam-implementation-05-verify` | Run full gate suite (check, test, format) and report evidence. |
...
- `workflow-gleam-implementation-01-scope` — **cannot be skipped.** Capturing
  intended behavior and public surface is required so later phases can be
  checked against it.
- `workflow-gleam-implementation-02-design` — **may be skipped only for trivial
...
- `workflow-gleam-implementation-03-implement` — **cannot be skipped.** This is
  the core code-writing phase.
- `workflow-gleam-implementation-04-test` — **cannot be skipped.** Every
  implementation must ship with tests covering happy path, error branches, and
  edge cases.
- `workflow-gleam-implementation-05-verify` — **cannot be skipped.** The full
...
| --- | --- |
| `workflow-gleam-implementation-05-verify` returns `outcome: pass` | `workflow-gleam-code-review-00-orchestration` |
...
The `workflow-gleam-implementation-05-verify` phase additionally returns the
verification-phase extra fields: `validations_run`, `constraints_checked`,
`evidence`, `failures`, `not_fully_checkable`. See
`workflow-gleam-implementation-05-verify`.
