...
> **STATUS: READY-TO-EXECUTE (design); implementation NEEDS-DEVSHELL**
> **Effort:** M
...
- [ ] [00-index.md](00-index.md), [../README.md](../README.md),
  ...
  this docs wave).
- [ ] `HomeAction::Init` gains `from: Option<String>` + positional
  `dest: Option<String>` at `cli_actions.rs:185`.
- [ ] Bare `home init` unchanged (ADR 0024d).
- [ ] `--config`/`--name` with `--from` or `dest` → usage error.
- [ ] Provisioning algorithm steps 1–10 implemented at `commands/home.rs:67`.
- [ ] `looks_like_git_url` made `pub(crate)` or moved to shared module
  (`config/registry.rs:126`).
- [ ] `workestrate.lock` generated by `config add`/`update`/`remove`/`home
  init`; save is atomic (mirror `registry.rs:667` test).
- [ ] Lock committed to home repo (not gitignored); absent `version` = oldest;
  newer-than-supported = hard error.
- [ ] Test plan §4 green via `nix develop`.
