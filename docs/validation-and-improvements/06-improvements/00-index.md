...
| [09-microsandbox-agentd-offline-build.md](09-microsandbox-agentd-offline-build.md) | microsandbox-filesystem agentd offline build (ADR 0011 carrier) | `MERGED TO FORK AS PR #1 (b43d7522, 2026-08-04); upstream superradcompany PR still pending; interim nix-side patch still in place. PR PREPARED, READY TO OPEN (branch fix/filesystem-agentd-path-override amended to bc7640b8 (8-point review hardening + tests, 2026-08-01); docs in .tmp/msb-upstream/; force-push + open pending user; interim nix-side patch machinery re-based onto the 0.6.15 fork workspace (microsandbox-fork flake input @ 79dc8a19 — the fork carries the MSB_AGENTD_PATH fix natively); option 2 REVERSED per ADR 0011 addendum; option 3 NEEDS-DEVSHELL + HOST-NIX)` | None file-level on other improvement specs; option 3 cross-references [01](01-mount-filtering-shadowing.md) | option 1 = **M** (incl. upstream review latency); option 2 = REVERSED; option 3 = **M** | `HOST-NIX` (option 3 build); option 1 is upstream |
