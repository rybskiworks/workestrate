...
> **Annotated 2026-08-01 (final model):** the map form this spec introduced SURVIVES as the one unified env map, but `{ secret }` binding semantics now include `bound` (default `host` = placeholder; `guest` = real value) and the `true` sugar exists for same-name placeholders — see [16-unified-secret-env-model.md](16-unified-secret-env-model.md). History kept.
