...
| [`06-improvements/12-per-instance-addressing.md`](06-improvements/12-per-instance-addressing.md) | Per-instance addressing + discovery-lite (executes ADR 0026; supersedes ADR 0021 §5 `--port-offset`) | `STATUS: IMPLEMENTED (Waves 1+2 landed; refuse-only/no-auto-start SUPERSEDED 2026-08-01 by the ADR 0026 addendum default-on lifecycle; --use retained; E1 NEEDS-KVM)` |
| [`06-improvements/13-secret-env-shorthand.md`](06-improvements/13-secret-env-shorthand.md) | Config ergonomics: string-or-table shorthand for `secret_env` (additive; `schema_version` stays 1) | `STATUS: EXECUTED (2026-07-31; commits a349d03, 1e7dc25)` — SUPERSEDED 2026-08-01 (v2 model 1ed2e6d removed `secret_env`; shorthand survives only in the v1 shim); shorthand form superseded by the final model's `true` sugar (16, 2026-08-01) |
| [`06-improvements/14-env-map-form.md`](06-improvements/14-env-map-form.md) | Config ergonomics: map form for `env` entries (formatting collapse; additive; `schema_version` stays 1) | `STATUS: EXECUTED (2026-07-31; commits 1035073, 564deca)` — map form became the v2 unified binding map (1ed2e6d); map survives as the unified map with `bound` in the final model (16, 2026-08-01) |
...
| [`06-improvements/16-unified-secret-env-model.md`](06-improvements/16-unified-secret-env-model.md) | Final unified secret/env model: one env map, per-binding `bound`, `allowed_hosts` rename, `schema_version` stays 1 (supersedes intermediate v2 of 1ed2e6d) | `STATUS: READY-TO-EXECUTE (design locked 2026-08-01)` |
...
| [`21-image-build-lifecycle.md`](06-improvements/21-image-build-lifecycle.md) | Image build/load lifecycle (ensure-images, change detection, selectors, reserved build dir) | `STATUS: IMPLEMENTED (phases A–E landed 2026-08-02/03; phase F multi-repo migration pending; ADR 0028 CWD independence)` |
