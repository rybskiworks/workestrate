...
| **Sub-steps** | **(a) FIRST — preserve the personal config (AMENDED by spec 10):** under the working-copy model, move/copy `.workestrate/repos/personal` into the new home as THE working copy (`~/.workestrate/repos/personal` — or `config-repos/personal` after the spec-10 rename); verify `git log --oneline -2` shows `c41a707` on `main`. NO standalone sibling. **(b) AMENDED by spec 10:** `cp -a` bundle → `~/.workestrate`; drop non-standard `scratch/` (RESOLVES Step 0(b) scratch/cache: neither — drop it); edit `~/.workestrate/config.toml` (`url` → the in-home working copy path OR the remote when added; `rev` → `c41a70736cd3f65032f6c4e5351302694251fef0` — clears the registry-rev staleness); set origin on `~/.workestrate/repos/personal` to the real remote when ready (NOT a standalone sibling); verify `workestrate config list` + `validate-config` in `nix develop` (runnable in this container via the store-path prefix; bare shell lacks `cc`). **(c)** One commit: `git rm scripts/local-xdg.sh scripts/migrate-xdg-to-repo.sh`; remove the `WORKESTRATE_HOME` line from `.envrc`; remove `/.workestrate/` (+ its comment) from `.gitignore`. **(d)** ONLY after (b) verified: `rm -rf /home/node/Development/ai-workbench/.workestrate`; sweep `rg -l '\.workestrate' -g '!docs/**' -g '!*.lock' .`. **(e) CODE (runnable in this container via `nix develop`, store-path prefix; bare shell lacks `cc`):** remove the discovery tier (`paths.rs:116-139`), `HomeKind::Discovered`, `emit_untrusted_discovery_warn`; KEEP `is_dir_trusted_via_base_registry` (second caller `trust.rs:327`); gates `cargo fmt --check` + `cargo clippy -- -D warnings` + `cargo test`. **(f)** ADR 0023 addendum + `60-glossary.md` tool-home entry. **(g)** compose/mount guidance: shadow-mount `~/.workestrate` ro at an explicit non-default guest path; the repo mount carries no bundle. |
| **Gate / exit criteria** | (a) in-home working copy exists at `~/.workestrate/repos/personal` with `c41a707` as HEAD (AMENDED by spec 10). (b) `workestrate config list` with NO `WORKESTRATE_HOME` export shows `personal` @ `c41a707` from `~/.workestrate`. (c) machinery commit landed. (d) bundle deleted; reference sweep clean. (e) cargo gates green in devshell. New precedence documented: **flag (`--home`) > env > legacy XDG > default `~/.workestrate`** (no discovery). |
| **Env marker** | (a)(c)(d)(f)(g): `verifiable-here`. (b) verify + (e): `verifiable-here` via `nix develop` (store-path prefix; bare shell lacks `cc`; verified 2026-07-29: `cc --version` → gcc 15.2.0 inside `nix develop`). |
...
| **Sub-steps** | **Option 2 (INTERIM CARRIER, ADR 0011 — first):** fork to `github:georgrybski/microsandbox-filesystem`, apply the patch on the fork, point `[patch.crates-io]` at the fork git URL, then delete `nix/packages/microsandbox-filesystem-patched.nix` + the patch file + the `_setup_vendor_link` devshell hook (`nix/devshells/default.nix:132-158`) + the agentctl.nix vendor staging (`nix/packages/agentctl.nix:59-84` adjusted) + justfile vendor-unlock/lock recipes (`justfile:155-174`). **Option 1 (UPSTREAM FIX — preferred end-state):** contribute an MSB_HOME-based agentd check to `crates/filesystem/build.rs` mirroring the SDK crate's own pattern (precedent: upstream #704); after an upstream release containing the fix, delete the fork + all compensation machinery and update the `control/agentctl/Cargo.toml` pin. **Option 3 (VERSION BUMP — separate combinable track):** `=0.5.6` → `=0.6.8` (`control/agentctl/Cargo.toml:17`); requires rewriting the agentd patch against 0.6.x build.rs (insertion point immediately before the new `if local.is_file() { copy_agentd(&local, &dest); return; }` branch — the 0.5.6 CI-gate anchor was deleted by upstream PR #1019) and re-validating the `msb --version` match check (`nix/packages/agentctl.nix:67-70`); 0.5.8+ added guest-write quotas and 0.6.6 added RESOLVE_BENEATH symlink protection (cross-check [06-improvements/01](06-improvements/01-mount-filtering-shadowing.md)); NEVER the yanked 0.6.5. |
| **Gate / exit criteria** | Option 2: `[patch.crates-io]` points at fork git URL; deleted machinery listed above; `nix flake check` green. Option 1: upstream PR merged (referencing #704 precedent) + released; machinery deleted; Cargo.toml pin updated. Option 3: `=0.6.8` pin; rewritten patch applies and build passes offline; `msb --version` match re-validated; spec 01 cross-checked for RESOLVE_BENEATH interaction. |
| **Env marker** | Option 2: `HOST-NIX` (build). Option 1: upstream-latency-bound (no local env gate beyond the post-merge deletion + `nix flake check`). Option 3: `NEEDS-DEVSHELL` + `HOST-NIX` (patch rewrite + cargo build/test + runtime `msb --version` re-validation). |
| **Parallelization** | Independent of Lane A/B and of Steps 0–8a — runs in parallel. Option 2 is **BLOCKED on fork push access** (`github:georgrybski/microsandbox-filesystem`). Option 3 is `HOST-NIX` and combinable with 1 or 2. Option 1 is upstream-latency-bound. Recommended sequence: option 2 (once unblocked) → option 1 → delete fork + machinery after release; option 3 separate track. |
...
- [ ] **Repo-local home retired** — [06-improvements/08](06-improvements/08-no-repo-local-home.md) executed end-to-end: personal config preserved as the in-home working copy at `~/.workestrate/repos/personal` (@ `c41a707`) — AMENDED by spec 10 (no standalone sibling), real home at `~/.workestrate` (registry rev fixed), machinery commit landed, bundle deleted, discovery tier removed from `paths.rs` (cargo gates green in devshell), ADR 0023 addendum + glossary updated. **(NOT STARTED — interim warning applies until step (a))**
...
- [ ] **Config repos as working copies + dotfiles home (spec 10)** — [06-improvements/10](06-improvements/10-config-repos-as-working-copies.md) executed: docs/decision landed here; spec 08 step (a) + `03-sibling-config-setup.md` topology carry AMENDED-by-spec-10 markers; code tasks (Task 1 `config-repos/` rename, Task 2 dirty-guard regression test, Task 3 `home init` scaffolding) landed with cargo gates green in `nix develop` (runnable in this container via the store-path prefix; bare shell lacks `cc`). **(NOT STARTED — code tasks run via `nix develop`)**
