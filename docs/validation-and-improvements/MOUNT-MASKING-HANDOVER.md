...
> Cross-references: STATUS.md, NEXT-SESSION.md, BEADS.md, spec 22 (06-improvements/22-dynamic-mount-masking-policy.md), spec 23 (06-improvements/23-microsandbox-fork-nix-flake-packaging.md), ADR 0028 (50-decisions/0029-policy-scopes-collect-and-compile.md).
...
- 16 commits in 43cfcbc..56e2684 (14 policy commits listed below + e32a97b `docs(handover): mount-masking feature handover` + 56e2684 `chore(beads): add mount-masking handover issues`; the 14 policy commits are the ones to assess):
  1. 5ca39fc docs(specs): 22 dynamic mount masking policy + ADR 0029; spec 01 → fallback
...
- **Spec 22 + ADR 0029 authoring** (5ca39fc): the dynamic mount masking policy spec and the collect-and-compile ADR; spec 01 dispositioned to SECONDARY/FALLBACK.
...
- **Collect-not-merge (ADR 0029)**: fragments collected per layer, compiler owns precedence/freeze/trust/provenance; merge.rs never sees policy; mounts vec wholesale-replace unchanged.
...
- ADR 0028: `docs/migration/50-decisions/0029-policy-scopes-collect-and-compile.md` (123 lines)
...
1. **Read the design**: spec 22 (`docs/validation-and-improvements/06-improvements/22-dynamic-mount-masking-policy.md`) + ADR 0028 (`docs/migration/50-decisions/0029-policy-scopes-collect-and-compile.md`) + spec 23 (`06-improvements/23-microsandbox-fork-nix-flake-packaging.md`).
...
|---|---|---|---|
| Collect-not-merge (ADR 0029) | workestrate `loading.rs:407-486` (collect), `compile.rs` (compile) | `cargo test` mount_policy + config | **verified** (unit) |
