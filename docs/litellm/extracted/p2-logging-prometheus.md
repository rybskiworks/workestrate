...
- CRITICAL: Prometheus metrics work WITHOUT a database and WITHOUT Redis (for basic metrics). `litellm_settings.callbacks: ["prometheus"]` + `GET /metrics` endpoint work in-memory. The workestrate can use Prometheus for request counting, latency, LLM provider metrics, deployment state, fallback metrics, and proxy-level tracking. Budget/rate-limit metrics (`litellm_remaining_team_budget_metric`, `litellm_remaining_team_budget_metric`, etc.) require DB-backed virtual keys/teams (unavailable). `prometheus_initialize_budget_metrics` is moot without DB. `require_auth_for_metrics_endpoint: true` works without DB (uses master_key auth). `custom_prometheus_tags` with `User-Agent: claude-cli/*` is directly relevant for workestrate agent tracking. Multi-worker requires `PROMETHEUS_MULTIPROC_DIR` (single-process in-memory doesn't need it).
