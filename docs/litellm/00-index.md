...
OpenRouter, Bedrock, etc.). Workestrate runs LiteLLM **in-memory inside a
...
| `workestrate-recommended-patterns.md` | Workestrate in-memory deployment patterns and project choices | exists |
...
- DB-gated features unavailable: virtual keys, teams, users, budgets, spend
  ...
  CRUD. See `schemas/config-yaml.normalized.schema.md` → "Workestrate in-memory
  deployment mapping".
