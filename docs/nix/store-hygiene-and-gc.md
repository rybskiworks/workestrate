...
### Git hooks vs GC (pre-commit / pre-push shims)

A previous `prek install` wrote `.git/hooks/pre-commit` shims that exec'd a hardcoded store-path prek binary against a generated `.pre-commit-config.yaml`. GC deleted both, so every commit failed unless passed `--no-verify`. The fix: both hooks are now pure-sh shims (canonical tracked copies `scripts/git-hooks/pre-commit.sh` and `scripts/git-hooks/pre-push.sh`; reinstall with `cp scripts/git-hooks/pre-commit.sh .git/hooks/pre-commit && chmod +x .git/hooks/pre-commit` and `cp scripts/git-hooks/pre-push.sh .git/hooks/pre-push && chmod +x .git/hooks/pre-push`) that never reference store paths. The generated `.pre-commit-config.yaml` is gitignored and never committed. Inside a devenv shell the declared `git-hooks.hooks.*` (via the nix-tooling devenv modules) enforce with the pinned toolchain; hook exactness otherwise comes from `nix flake check` / CI, not the commit gate.

The shims implement three tiers, in order:

1. **Tier 1 (pre-commit, bare-shell-safe):** the unconditional secret-material grep (`*.agekey`, `age.txt`, `*.pem`, `id_rsa*`, `.env` — POSIX grep only, always runs, never skips) plus fast linters over staged files (`typos`, `nixfmt`, `statix`, `deadnix`) and the `tombi` TOML gates, each with present-run / absent-skip-with-message semantics (skip messages go to stderr and never fail the commit).
2. **Tier 2 (pre-commit, in-devshell):** `rustfmt --edition <crate> --check` over staged `.rs` files when `rustfmt` is on PATH; the edition is read from the crate's `Cargo.toml` (`./Cargo.toml` or `./control/agentctl/Cargo.toml`, default 2021) — 2024 would break pre-2024 crates (`gen` becomes reserved). Absent rustfmt or no manifest → skip with message.
3. **Tier 3 (pre-push):** full `just check` (cargo fmt --check + clippy `-D warnings` + cargo check). clippy compiles the crate, so it is too heavy for pre-commit — pre-commit gets rustfmt-only, and the compile-grade gate runs once per push. Outside a nix-capable env (no `just`/`nix`, or no `check` recipe) the hook prints one skip message to stderr and exits 0.

Canonical note (all repos): a fresh clone ships no hooks, so plain commits work until one is installed. Entering the devenv shell would run git-hooks.nix's installer on every entry: it moves any foreign `.git/hooks/pre-commit` to `.git/hooks/pre-commit.legacy` and installs a generated hook that execs a store-path'd pre-commit against the generated `.pre-commit-config.yaml` — the pure-sh fallback would be clobbered, its secret gate lost in-shell, and the generated hook would dangle after GC. This clobber is DISABLED by default via nix-tooling `devenvModules/base.nix` (`git-hooks.install.enable = false`); the note applies only if a consumer re-enables installation. Outside the shell (or after GC/shell entry, if re-enabled) reinstall the fallbacks (`cp scripts/git-hooks/pre-commit.sh .git/hooks/pre-commit && chmod +x .git/hooks/pre-commit`, and likewise for `pre-push.sh`); their skip messages are audible (stderr) and the secret-material gate is unconditional.
