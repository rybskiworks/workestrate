---
type: Crawl Source
title: "NixOS — Configuration Syntax and Module System"
description: "NixOS configuration syntax, configuration file, abstractions, and modularity."
resource: https://nixos.org/manual/nixos/stable/#sec-configuration-syntax
tags: [nix, nixos-manual, configuration, module-system, configuration-nix]
timestamp: 2026-07-24T00:00:00Z
---

# Crawl: nixos-configuration-syntax
...
- fetch: 200
- version: NixOS 26.05
- feeds_docs: nixos-configuration.md, nixos-module-system.md
...
[NixOS Configuration File](</manual/nixos/stable/#sec-configuration-file>)
[Abstractions](</manual/nixos/stable/#sec-module-abstractions>)
[Modularity](</manual/nixos/stable/#sec-modularity>)
...
Sets can be nested, and in fact dots in option names are shorthand for defining a set containing another set. For instance, [`services.httpd.enable`](</manual/nixos/stable/options#opt-services.httpd.enable>) defines a set named `services` that contains a set named `httpd`, which in turn contains an option definition named `enable` with value `true`. This means that the example above can also be written as:
...
The latter option definition changes the default PostgreSQL package used by NixOS’s PostgreSQL service to 14.x. For more information on packages, including how to add new ones, see [the section called “Adding Custom Packages”](</manual/nixos/stable/#sec-custom-packages> "Adding Custom Packages").
...
Note that both `configuration.nix` and `kde.nix` define the option [`environment.systemPackages`](</manual/nixos/stable/options#opt-environment.systemPackages>). When multiple modules define an option, NixOS will try to _merge_ the definitions. In the case of [`environment.systemPackages`](</manual/nixos/stable/options#opt-environment.systemPackages>) the lists of packages will be concatenated. The value in `configuration.nix` is merged last, so for list-type options, it will appear at the end of the merged list. If you want it to appear first, you can use `mkBefore`:
...
For other types of options, a merge may not be possible. For instance, if two modules define [`services.httpd.adminAddr`](</manual/nixos/stable/options#opt-services.httpd.adminAddr>), `nixos-rebuild` will give an error:
...
For example, here is a module that adds some packages to [`environment.systemPackages`](</manual/nixos/stable/options#opt-environment.systemPackages>) only if [`services.xserver.enable`](</manual/nixos/stable/options#opt-services.xserver.enable>) is set to `true` somewhere else:
