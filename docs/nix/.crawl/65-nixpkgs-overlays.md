...
description: "Overlays in nixpkgs: installing, defining, and using them."
resource: https://nixos.org/manual/nixpkgs/stable/#sec-overlays
tags: [nix, nixpkgs-manual, overlays]
timestamp: 2026-07-24T00:00:00Z
---

# Crawl: nixpkgs-overlays
...
[Installing overlays](</manual/nixpkgs/stable/#sec-overlays-install>)
[Defining overlays](</manual/nixpkgs/stable/#sec-overlays-definition>)
[Using overlays to configure alternatives](</manual/nixpkgs/stable/#sec-overlays-alternatives>)
...
[Set overlays in NixOS or Nix expressions](</manual/nixpkgs/stable/#sec-overlays-argument>)
[Install overlays via configuration lookup](</manual/nixpkgs/stable/#sec-overlays-lookup>)
...
The list of overlays is determined as follows.
  ...
  1. First, if an [`overlays` argument](</manual/nixpkgs/stable/#sec-overlays-argument> "Set overlays in NixOS or Nix expressions") to the Nixpkgs function itself is given, then that is used and no path lookup will be performed.
...
Overlays are similar to other methods for customizing Nixpkgs, in particular the `packageOverrides` attribute described in [the section called “Modify packages via `packageOverrides`”](</manual/nixpkgs/stable/#sec-modify-via-packageOverrides> "Modify packages via packageOverrides"). Indeed, `packageOverrides` acts as an overlay with only the `prev` argument. It is therefore appropriate for basic use, but overlays are more powerful and easier to distribute.

## Using overlays to configure alternatives 

[BLAS/LAPACK](</manual/nixpkgs/stable/#sec-overlays-alternatives-blas-lapack>)
[Switching the MPI implementation](</manual/nixpkgs/stable/#sec-overlays-alternatives-mpi>)
...
All programs that are built with [MPI](<https://en.wikipedia.org/wiki/Message_Passing_Interface>) support use the generic attribute `mpi` as an input. At the moment Nixpkgs natively provides the following MPI implementations:
  ...
  * [Open MPI](<https://www.open-mpi.org/>) (default), attribute name `openmpi`
