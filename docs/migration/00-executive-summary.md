...
1. **Homeless-registry recursion.** If config repos are gitignored children
   inside the workspace repo (`config.d/<name>/`), the registry of *which*
   config repos exist has no tracked home. A second machine cannot reconstruct
   the set of config repos without out-of-band knowledge. The recursion
...
|---|---|---|---|
| **Registry** | `~/.config/workestrate/config.toml` | Tool settings, config-repo registry (name→url→ref→rev), ordered `layers` list, `[trusted_projects]` | User's dotfiles repo |
| **Config repos** | `~/.local/share/workestrate/repos/<name>/` | `workestrate.toml`, `.env.enc`, `.sops.yaml`, `infra/litellm/`, `agents/*/config/` | Each config repo's own git |
| **State** | `~/.local/state/workestrate/` | `sources/<name>/` (agent source checkouts + builds), `workspaces/`, `var/` | Not tracked (runtime state) |

The registry is the **home** for config-repo references — it IS the user's
dotfiles. This is the kubeconfig model: `~/.kube/config` holds cluster
references; `~/.config/workestrate/config.toml` holds config-repo references.
...
for config repos. Flake materialization is optional sugar for team repos.
...
- **SOPS+age secrets infrastructure.** `setup-secrets.sh`, `decrypt-env`,
  `write-env` wrappers stay in core; they target a config repo by name or path.
