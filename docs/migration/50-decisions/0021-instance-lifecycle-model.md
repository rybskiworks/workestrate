...
**Instance id slug rules** (ASSUMPTION (impl): the implementation track
enforces these at parse time; if it relaxes them, this section is the
authoritative contract and the impl must be reconciled to it):
...
- `--port-offset` is **not** persisted; it is per-invocation. To run a
  long-lived canary on an offset port, the operator (or agent) passes it on
  every `up`/`logs`/`down` for that instance, or pins it via the instance id
  (ASSUMPTION (impl): the impl may persist the offset in the port-registry
  record so `down`/`logs`/`ps` can recover it without re-passing the flag;
  this ADR does not require either choice but recommends persistence for
  ergonomics).
...
uses (ASSUMPTION (impl): the impl derives `JsonSchema` on `ConfigFile` and
sub-structs via `schemars`; if the impl uses a hand-maintained schema, the
drift guard below still applies but the "single source of truth" claim is
weakened — flag for reconciliation).

- The generated schema is committed at
  `control/agentctl/schema/workestrate.toml.json` and regenerated by a
...
- **Hand-maintained schema (vs schemars-derived):** rejected for the same
  ...
  (ASSUMPTION (impl): if the impl cannot derive `JsonSchema` cleanly, a
  hand-maintained schema + the drift guard is the fallback; the drift
  guard is the load-bearing piece, not the derivation method.)
