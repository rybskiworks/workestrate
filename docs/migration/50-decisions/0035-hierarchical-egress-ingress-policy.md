...
> **2026-09-04 removal note:** the `entitlements` mechanism (`workloads.<name>.entitlements`, `ALLOWED_ENTITLEMENTS`, the merge/validate allow-gates) is REMOVED in full — deleted from types, validation, merge, fixtures, reference config, scaffold/jinja templates, SPEC, and the personal prime/tempest capsules. Rationale: no public release and no external consumers (nothing to stay compatible with), and the remaining controls suffice — home `final` seals still veto via the policy ladder, review covers the rest, and `plan` now renders a loud `NOTE: workload '<name>' runs relaxed defaults (egress|ingress=allow)` per relaxed workload as the grep-able visibility surface. Semantics going forward: monotonic-deny-upward is GONE by design; an explicit `allow` at the winning layer stands alone with no magic word; home `final` seals still veto; absent = deny is unchanged. E1 (2026-09-04): the defaults axis is now sealed too — the effective `[network.defaults]` allow rides the ladder freeze walk as a synthetic lowest-rung allow-all, so a home `deny.all=true final=true` (or any covering final deny, coarse FIX1 parity) freezes the relaxed default per `on_conflict` (sealed ⇒ deny stands; the workload's own final deny does not self-freeze), and an existing `defaults = "allow"` under a home final deny is thereby silently sealed — a pre-release behavior change, accepted. The historical sections below are left intact as history.
