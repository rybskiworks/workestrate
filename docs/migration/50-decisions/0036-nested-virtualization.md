...
> **2026-09-06 amendment — D1 REVERSED (env-gated fork).** The always-on port (fork b2c672c8, upstream #823 shape: `nested_virt(true)` hardcoded) contradicted the D2/D3 semantics: the host-KVM checks were cosmetic at the CPU level because every guest — `nested=off` included — got the nested CPU capability regardless. D1 is therefore reversed: the VMM flag is now env-gated behind a process-env flag (default OFF; only the exact value `1` enables it) in the fork at b1424618 (`crates/runtime/lib/vm.rs` `build_vm()` reads `.nested_virt(nested_virt_enabled())`), and agentctl a7231be drives that var from the `nested_up_decision` in the pre-create gate (`check_nested_up_gate` now returns `Result<bool>`; `up` sets the env flag to `1` exactly when the decision resolves nested-ON — `require` passing its probes, or `prefer` on a host that affirmatively offers nested — and REMOVES the var otherwise; refusals unchanged). The §8 "Always-on CPU … documented limitation, D1" risk entry is CLOSED effective with the pin bump to b1424618 — pending the host nix relock; the live flake pin is still b2c672c8 (always-on) until then, so the limitation still describes the currently pinned build. The historical sections below are left intact as history.
>
> **Continuation (same date) — the env gate is SUPERSEDED by the first-class spec-field option.** Fork 09aeb8ae (feat) + 8e53de7e (docs) make nested virtualization a first-class spec option: `resources.nested_virt` (default off, serialized only when true) now travels spec → SDK builder (`SandboxBuilder::nested_virt(bool)`) → launch JSON → VmConfig → VMM flag, plus the `msb run/create --nested-virt` CLI flag. agentctl now threads the gate verdict via `builder.nested_virt(nested_on)` (per-sandbox spec data, not process-global state — the ambient-shell-env leak and the shared-env-key race are eliminated), and the env mechanism is DELETED on both sides (zero remaining references). D1's reversal now lands as a first-class fork option, not a process-global env. The §8 risk entry remains "CLOSED effective with the pin bump", now to 8e53de7e — the live flake pin is STILL b2c672c8 until the host relock, so the limitation still describes the currently pinned build.
**References:** `control/agentctl/src/config/types.rs:1304-1387` (`NestedMode` / `VirtualizationConfig` / `VirtualizationPolicyFragment`); `control/agentctl/src/microsandbox/nested.rs` (probe + `resolve_virtualization` + `nested_up_decision`); `control/agentctl/src/microsandbox/plan.rs:93-102` (`VirtualizationPlan`); `control/agentctl/src/microsandbox/workload/config.rs:430-494` (`plan_virtualization_for`); `control/agentctl/src/microsandbox/runtime/run.rs:873-917,1293-1311` (pre-create gate + `apply_nested_virt` builder-option wiring); `control/agentctl/src/commands/doctor.rs:103-158` (`doctor_check_nested_virt`); `control/agentctl/src/main.rs` (`Commands::Msb`); `control/agentctl/src/merge.rs:413-452` (`VirtualizationLadder`); `control/agentctl/src/config/loading.rs:999-1046` (ladder collect); `control/agentctl/src/microsandbox/provenance.rs:58-61` (hash exclusion); `scripts/host-check.sh:37-54`; `scripts/kvm-tests.sh:139-168`
...
- **D1 — Accept always-on fork + honest docs.** **(SUPERSEDED — reversed by the 2026-09-06 amendment above: the always-on flag made the host-KVM checks cosmetic at the CPU level; the flag is now a first-class `resources.nested_virt` spec option, default off, in fork 09aeb8ae/8e53de7e, driven by `nested_up_decision` via `builder.nested_virt(...)` — historically env-gated default-off in b1424618 the same day. Original text retained for history.)** No fork-side flag (a permanent rebase tax on the exact file upstream touches, for marginal benefit while the firmware lacks `CONFIG_KVM`). Honesty rule, stated in errors/doctor/SPEC: `nested=off` does NOT remove CPU capability — it withholds the device promise. Backlog V2 gated-fork if the threat model ever needs true CPU gating.
...
Always-on CPU for all guests incl. `nested=off` (documented limitation, D1) — **CLOSED 2026-09-06 (D1 reversed, see amendment): the spec-field option (default OFF; fork 09aeb8ae/8e53de7e, superseding the same-day env gate) removes the CPU capability once the pin bumps to 8e53de7e; the entry still applies to the live pin b2c672c8 until the host relock.** `prefer` warn-fatigue masking real misconfig (distinct warn vs note severities + doctor summary mitigate). Atomic install ≠ atomic migration: first new-binary run mutates shared `msb.db` — quiesce + snapshot pre-bump, never auto-migrate unattended. Crash/partial-sandbox discipline at `up`-refusal (no leftovers — asserted in tests).
