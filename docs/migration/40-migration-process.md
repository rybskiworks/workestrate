...
succeeds on KVM host (HOST-KVM gate). Root `workestrate.toml` is removed from
the tool repo; `config.reference/` is the synthetic fallback.
...
### Step 3.6: Full validation — DONE (post-strip-down)
- `just verify` passes: fmt, clippy, check, test (34 passed), litellm-check, golden-check (3 synthetic workloads MATCH)
- Cargo.lock stability: PASS (after revert)
- `config.reference/` is synthetic; root user deployment files stripped from repo
