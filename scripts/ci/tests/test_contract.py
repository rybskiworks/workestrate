...
class SelectionTests(unittest.TestCase):
    ...
    def test_unknown_missing_and_empty_changes_fail_closed(self):
        for files in [None, [], ['new-directory/test.rs'], ['.github/workflows/ci.yml'],
                      ['docs/migration/state.md'], ['README.md']]:
            self.assertEqual(selection.select('pull_request', {}, files)['code'], 'true')
        for files in [None, []]:
            self.assertEqual(selection.select('pull_request', {}, files)['nix'], 'true')

    def test_nix_changes_select_functional_checks_without_label(self):
        for path in ['flake.lock', 'flake.nix', 'nix/new.nix', 'templates/config.toml',
                     'config.reference/workestrate.toml', '.github/workflows/ci.yml']:
            self.assertEqual(selection.select('pull_request', {}, [path])['nix'], 'true')

    def test_merge_queue_proves_code_and_nix_but_not_manual_build(self):
        self.assertEqual(selection.select('merge_group', {}, ['docs/design.md']),
                         {'code': 'true', 'nix': 'true', 'build': 'false'})
    ...
    def renamed_paths(self, old, new):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(git('diff', '--name-only', '-z', f'{base}..{head}', '--'), new.encode() + b'\0')
        ...
        for old, new in [('control/agentctl/src/lib.rs', 'docs/lib.rs'),
            ...
            with self.subTest(old=old):
                for event, paths in self.renamed_paths(old, new).items():
                    ...
                    self.assertEqual(result['nix'], 'true' if event == 'push' or old.endswith('.nix') else 'false')
...
class GateTests(unittest.TestCase):
    def needs(self, code='true', nix='false', build='false'):
        ...
        result = {name: {'result': 'success'} for name in gate.ALWAYS}
        result['changes']['outputs'] = flags
        result.update({name: {'result': 'success' if flags[flag] == 'true' else 'skipped'}
                       for name, flag in gate.CONDITIONAL.items()})
        return result
    ...
    def test_selected_nix_and_build_may_not_skip(self):
        for job in ['e2e-nix', 'heavy-build']:
            needs = self.needs('true', 'true', 'true')
    ...
    def test_missing_dependency_and_bad_flag(self):
        ...
        with self.assertRaises(ValueError):
            gate.validate(needs)
        needs = self.needs()
        needs['changes']['outputs']['code'] = 'unknown'
        with self.assertRaises(ValueError):
            gate.validate(needs)
...
class MetadataTests(unittest.TestCase):
    ...
    def test_lock_mismatch(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self.fixture(root, locked='0.0.9')
            with self.assertRaises(ValueError):
    ...
    def test_invalid_version(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            self.fixture(root, version='01.2.3')
            with self.assertRaises(ValueError):
                metadata.validate(root)
