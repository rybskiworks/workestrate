...
pick_editor() {
  ...
  fail "no editor found; set EDITOR to an absolute path or run inside 'nix develop' (which provides nano and vim)"
}
...
cmd_init() {
  ...
  log "You can now run: run-with-secrets litellm up"
}
