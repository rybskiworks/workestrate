...
pub fn load_secrets() -> Result<()> {
    ...
    let secret_file =
        std::env::var("SECRET_FILE").unwrap_or_else(|_| ".env.enc".to_string());
    ...
    if std::env::var("SOPS_AGE_KEY_FILE").is_err() {
        if let Some(home) = std::env::var_os("HOME") {
            let key_path = std::path::Path::new(&home)
                .join(".config/sops/age/ai-workbench-secrets.txt");
    ...
    let output = Command::new("sops")
        ...
        .output()
        .map_err(|e| {
            anyhow::anyhow!("failed to run 'sops decrypt' (is sops on PATH?): {e}")
        })?;
    ...
    let values: HashMap<String, String> =
        serde_json::from_slice(&output.stdout).map_err(|e| {
            anyhow::anyhow!("failed to parse sops JSON output: {e}")
        })?;
