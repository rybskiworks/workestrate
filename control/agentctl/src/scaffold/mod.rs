...
//! destination directory to bootstrap a new workestrate-config repo.
//!
//! Design constraints:
...
mod tests {
    ...
    fn render_all_substitutes_config_name() {
        ...
        assert!(
            toml_entry.contains("workestrate config: personal"),
    ...
    fn empty_renders_clean_header() {
        ...
        // The comment line must be clean: `# workestrate config: testrepo`
        // on its own line, not merged with the next.
        assert!(
            rendered.contains("# workestrate config: testrepo\n"),
