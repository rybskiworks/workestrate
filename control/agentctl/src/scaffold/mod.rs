//! Scaffold templates for `workestrate fleet new`.
//!
//! Embedded at compile time via `include_str!` so the binary stays
//! self-contained (no runtime file lookup, no CWD dependence). The
//! `workestrate fleet new` command renders these templates into a
//! destination directory to bootstrap a new workestrate-fleet.
//!
//! Design constraints:
...
pub const DEFAULT_COPIER_SRC_PATH: &str =
    "github:georgrybski/ai-workbench/templates/workestrate-config";
...
/// Mirrors the clap `default_value` on `FleetAction::New::core_flake_url`
...
const T_SCHEMA_REGISTRY_JSON: &str = include_str!("../../../../schemas/registry.schema.json");
...
pub struct ScaffoldVars {
    /// `[a-z0-9-]` fleet name (validated upstream).
...
/// schemas `workestrate.schema.json`, `workestrate-workload.schema.json`,
/// and `registry.schema.json`)
/// shared by both the full scaffold ([`render_all`]) and the `--empty`
/// minimal scaffold in `commands::fleet_cmd`. Single source of truth so the
...
pub(crate) fn tombi_files() -> Result<Vec<(&'static str, String)>> {
    Ok(vec![
        ...
        (
            ...
            T_SCHEMA_REGISTRY_JSON.to_string(),
        ),
    ])
}
...
mod tests {
    ...
    fn render_all_includes_required_files_no_flake() {
        ...
        assert!(names.contains(&"schemas/registry.schema.json"));
        assert!(
    ...
    fn render_all_substitutes_config_name() {
        ...
        assert!(
            toml_entry.contains("workestrate fleet: personal"),
    ...
    fn render_all_tombi_and_schema_content() {
        ...
        let tombi = out
            ...
            .expect("tombi.toml");
        assert!(
            tombi.matches("[[schemas]]").count() == 3,
            "tombi.toml must carry three [[schemas]] mappings (full + workload subschema + registry); got:\n{}",
            tombi
        );
        assert!(
        ...
        assert!(
            tombi.contains("schemas/registry.schema.json"),
            "tombi.toml must reference the registry schema; got:\n{}",
            tombi
        );
        assert!(
            tombi.contains("config.toml"),
            "tombi.toml must map the registry schema to config.toml; got:\n{}",
            tombi
        );
        assert!(
        ...
        let registry_schema = out
            .iter()
            .find(|(n, _)| *n == "schemas/registry.schema.json")
            .map(|(_, c)| c.as_str())
            .expect("schemas/registry.schema.json");
        assert!(
            registry_schema.contains("Registry") || registry_schema.contains("\"$schema\""),
            "schemas/registry.schema.json must be the vendored registry JSON schema"
        );
    }
    ...
    fn empty_renders_clean_header() {
        ...
        //
        // We reconstruct the exact format! string used by cmd_fleet_new's
        ...
        // The comment line must be clean: `# workestrate fleet: testrepo`
        // on its own line, not merged with the next.
        assert!(
            rendered.contains("# workestrate fleet: testrepo\n"),
