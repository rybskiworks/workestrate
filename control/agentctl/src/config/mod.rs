...
pub use paths::{
    ...
    resolve_home_with_kind, resolve_state_dir, resolve_store_dir, source_store_dir,
    xdg_config_dir, xdg_data_dir, xdg_state_dir, HomeKind,
};
#[allow(unused_imports)]
pub use types::{
    BakedFileSpec, BinarySpec, ConfigFile, ConfigRepoEntry, Context, EnvVarConfig, ImageSpec,
    LocalBuildConfig, NetworkConfig, Registry, RegistrySettings, SecretDefConfig, SecretEnvConfig,
...
use types::CONFIG_FIELDS;
use types::WORKLOAD_FIELDS;
...
pub(crate) fn is_dir_trusted_via_base_registry(dir: &Path) -> bool {
    let path = paths::base_registry_path();
    if !path.exists() {
        return false;
    }
...
pub fn load_config() -> Result<ConfigFile> {
    ...
    if let Some(path) = paths::reference_config_path() {
        if path.exists() {
...
pub(crate) mod tests {
    ...
    // stays here until the migrate code itself moves in a later commit.
